...
Droidrun Portal exposes trigger management over the local `ContentProvider`, the local WebSocket JSON-RPC API, and the reverse WebSocket connection used by remote host control. The rule and run payloads use the same `TriggerJson` schema that Portal persists internally; there is no separate external trigger schema.
